...
class MasterDataResponseModel {
  ...
  factory MasterDataResponseModel.fromAPIResponseMap(Map<String, dynamic> json) {
    return MasterDataResponseModel(
      applicationAndSurveyList:
          List<ApplicationModel>.from(json['ApplicationAndSurveyList']?.map((x) => ApplicationModel.fromAPIResponseMap(x)) ?? []),
      traficTradeSitesList:
          List<TrafficTradesModel>.from(json['TraficTradeSitesList']?.map((x) => TrafficTradesModel.fromAPIResponseMap(x)) ?? []),
      userCityList:
          List<CityModel>.from(json['UserCityList']?.map((x) => CityModel.fromAPIResponseMap(x)) ?? []),
      nearestDepo: List<NearestDepoModel>.from(json['NearestDepo']?.map((x) => NearestDepoModel(x)) ?? []),
      tradeAreaType: List<TradeAreaTypeModel>.from(json['TradeAreaType']?.map((x) => TradeAreaTypeModel(x)) ?? []),
      dealerInvestmentType: List<DealerInvestmentTypeModel>.from(json['DealerInvestmentType']?.map((x) => DealerInvestmentTypeModel(x)) ?? []),
      gfbList: List<GFBModel>.from(json['GFBList']?.map((x) => GFBModel(x)) ?? []),
      recommendationList: List<RecommendationModel>.from(json['RecommendationList']?.map((x) => RecommendationModel(x)) ?? []),
      shiftHourList: List<ShiftHourModel>.from(json['ShiftHourList']?.map((x) => ShiftHourModel(x)) ?? []),
      hmlList: List<HMLModel>.from(json['HMLList']?.map((x) => HMLModel(x)) ?? []),
      ynList: List<MasterDataYNAModel>.from(json['YNList']?.map((x) => MasterDataYNAModel(x)) ?? []),
      ynnList: List<MasterDataYNAModel>.from(json['YNNList']?.map((x) => MasterDataYNAModel(x)) ?? []),
      nfrList: List<NFRModel>.from(json['NFRList']?.map((x) => NFRModel(x)) ?? []),
    );
  }
  ...
  Map<String, String> listTypeToMap(MasterListType type) {
    ...
    switch (type) {
      case MasterListType.nearestDepo:
        values = nearestDepo.map((e) => e.name).join(',');
        break;
      case MasterListType.tradeAreaType:
        values = tradeAreaType.map((e) => e.type).join(',');
        break;
      case MasterListType.dealerInvestmentType:
        values = dealerInvestmentType.map((e) => e.type).join(',');
        break;
      case MasterListType.gfbList:
        values = gfbList.map((e) => e.gfb).join(',');
        break;
      case MasterListType.recommendationList:
        values = recommendationList.map((e) => e.recommendation).join(',');
        break;
      case MasterListType.shiftHourList:
        values = shiftHourList.map((e) => e.shiftHour).join(',');
        break;
      case MasterListType.hmlList:
        values = hmlList.map((e) => e.hml).join(',');
        break;
      case MasterListType.ynList:
        values = ynList.map((e) => e.yna).join(',');
        break;
      case MasterListType.ynnList:
        values = ynnList.map((e) => e.yna).join(',');
        break;
      case MasterListType.nfrList:
        values = nfrList.map((e) => e.nfr).join(',');
        break;
    }
    return {
